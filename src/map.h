 ...
 */
typedef struct Map {
    int number_of_cities;  ///< liczba zalokowanych miast
    int number_of_routes;  ///< liczba zalokowanych dróg krajowych
    struct City **cities;  ///< tablica wskaźników na miasta
    struct Route **routes; ///< tablica wskaźników na drogi krajowe
} Map;
 ...
 * @param[in] built_year – rok budowy odcinka drogi.
 ...
 * @param[in] city1       – wskaźnik na napis reprezentujący nazwę miasta;
 * @param[in] city2       – wskaźnik na napis reprezentujący nazwę miasta;
 * @param[in] repair_year – rok ostatniego remontu odcinka drogi.
...
/** @brief Pokazuje rok ostatniego remontu odcinka drogi.
 * @param[in, out] map - mapa dróg.
 * @param[in] city1    - nazwa miasta.
 * @param[in] city2    - nazwa miasta.
 * @param[in] length   - długość odcinka drogi.
 * @return Rok ostatniego remontu odcinka drogi jeśli długość
 * się zgadza, INT_MAX, gdy długość się nie zgadza lub 0, gdy
 * miasta nie są połączone.
 */
...
/** @brief Dodaje odcinek między podanymi miestami do drogi krajowej.
 * Dodaje odcinek na końcu drogi krajowej lub tworzy drogę krajową
 * o podanym numerze i dodaje odcinek.
 * @param[in, out] map - mapa dróg.
 * @param[in] route_id - numer drogi krajowej.
 * @param[in] city1    - nazwa miasta.
 * @param[in] city2    - nazwa miasta.
 * @return Wartość @true, jeśli operacja się powiodła. Wartość @false,
 * gdy nie udało się zaalokować pamięci lub którys z parametrów
 * był niepoprawny.
 */
...
/** @brief Usuwa drogę krajową z mapy dróg.
 * Zwalnia z pamięci drogę krajową o podanym numerze.
 * @param[in, out] map - mapa dróg.
 * @param[in] route_id - numer drogi krajowej.
 * @return Wartość @true, jeśli operacja się powiedzie. Wartość
 * @false, gdy któryś z paramerów był niepoprawny lub nie istniała
 * droga o podanym numerze.
 */
